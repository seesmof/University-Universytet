...
constfloattoRadians = 3.14159265f / 180.0f;
float curAngle = 0.0f;
...
voidAddShader(GLuinttheProgram, constchar *shaderCode, GLenumshaderType)
{
...
intmain()
{
    ...
    CreateTriangle();
    CompileShaders();
    glm::mat4projection = glm::perspective(glm::radians(45.0f), (GLfloat)bufferWidth / (GLfloat)bufferHeight, 0.1f, 100.0f);
    while (!glfwWindowShouldClose(mainWindow))
    {
        ...
        // Очистка буфера кольору зеленим кольором
        glClearColor(0.0f, 1.0f, 0.8f, 1.0f);
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        glUseProgram(shader);
        glm::mat4model(1.0f);
        model = glm::translate(model, glm::vec3(triOffset, triOffset, -3.0f));
