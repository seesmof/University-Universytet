...
# Create network with 2 layers: 4 neurons in input layer (Competitive) and 2 neurons in output layer (linear)
net = nl.net.newlvq(nl.tool.minmax(inp), 4, [0.6, 0.4])
# Train network
error = net.train(inp, target, epochs=1000, goal=-1)
...
class1 = inp[target[:, 0] > 0]
class2 = inp[target[:, 1] > 0]
