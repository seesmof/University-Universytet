    ...
    try:
        for name, entries in temps.items():
            ...
            for entry in entries:
                print(
                    f"{entry.label or name}: {entry.current}°C (high = {entry.high}°C, crit = {entry.critical}°C)"
                )
            print()
    except:
        pass
...
def get_nics():
    ...
    for nic in sorted(
        psutil.net_if_addrs(),
        key=lambda nic: (io_all[nic].bytes_recv + io_all[nic].bytes_sent),
        reverse=True,
    ):
...
def get_proc(
    ...
    if sorting == "pid":
        ...
        sort_cmd = (
            "p.info['cpu_percent']"
            if proc_per_cpu
            else "(p.info['cpu_percent'] / cpu_count)"
        )
    else:
    ...
    print(
        f"{'Pid:':>7} {'Program:':<{prog_len}}",
        f"{'Arguments:':<{arg_len-4}}" if arg_len else "",
        f"{'Threads:' if arg_len else ' Tr:'} {'User:':<9}Mem%{'Cpu%':>11}",
        sep="",
    )
    ...
    for p in sorted(
        ...
        cpu = (
            p.info["cpu_percent"]
            if proc_per_cpu
            else (p.info["cpu_percent"] / psutil.cpu_count())
        )
        ...
        print(
            ...
            (
                ...
                if len(p.info["username"]) < 10
                else f"{p.info['username'][:8]:<8}+"
            ),
...
def proc_tree(
    ...
    def create_tree(
        parent: int, tree, indent: str = "", inindent: str = " ", found: bool = False
    ):
        ...
        if cont:
            print(
                f"{f'{inindent}{parent} {cleaned(name)}':<{tree_width}.{tree_width-1}}",
                sep="",
                end="",
            )
        if getinfo and cont:
            ...
            cpu = (
                getinfo["cpu_percent"]
                if proc_per_cpu
                else (getinfo["cpu_percent"] / psutil.cpu_count())
            )
            print(
                (
                    f"{getinfo['num_threads']:>4} "
                    if getinfo["num_threads"] < 1000
                    else "999> "
                ),
                (
                    f"{getinfo['username']:<9.9}"
                    if len(getinfo["username"]) < 10
                    else f"{getinfo['username'][:8]:<8}+"
                ),
                (
                    f"{getinfo['memory_percent']:>4.1f}"
                    if getinfo["memory_percent"] < 100
                    else f"{getinfo['memory_percent']:>4.0f} "
                ),
                f"{cpu:>11.1f} " if cpu < 100 else f"{cpu:>11.0f} ",
                sep="",
            )
...
def get_disks(exclude: str = None, filtering: str = None):
    ...
    if filtering:
        filtering: Tuple[str] = tuple(filtering.split(" "))
    io_counters = psutil.disk_io_counters(
        perdisk=True if system == "Linux" else False, nowrap=True
    )
    print("Ignored line")
    for disk in psutil.disk_partitions():
        disk_io = None
        disk_name = (
            disk.mountpoint.rsplit("/", 1)[-1] if not disk.mountpoint == "/" else "root"
        )
        while disk_name in disk_list:
            disk_name += "_"
        disk_list += [disk_name]
        if (
            excludes
            and disk.fstype in excludes
            or filtering
            and not disk_name.endswith(filtering)
        ):
            continue
        ...
        try:
            ...
            pass
        print(
            f"{disk.device} {disk_u.total >> 10} {disk_u.used >> 10} {disk_u.free >> 10} {disk_u.percent:.0f} ",
            end="",
        )
        try:
            if system == "Linux":
        ...
        if disk_io:
            disk_hist[disk.device] = (disk_io.read_bytes, disk_io.write_bytes)
