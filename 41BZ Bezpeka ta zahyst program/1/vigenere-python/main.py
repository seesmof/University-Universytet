...
def vigenere(text: str, key: str, alphabet: str, to_decode: bool = False) -> str:
    if not text or not key:
        return ""

    num_to_letter: dict[int, str] = dict(enumerate(alphabet))
    letter_to_num: dict[str, int] = {v: k for k, v in num_to_letter.items()}

    # Заміняє всі символи, яких немає в алфавіті
    non_letters_pattern: str = rf"[^{alphabet.upper()}{alphabet.lower()}]"
    clean_key: str = re.sub(non_letters_pattern, "", key).upper()
    ...
    for letter in text:
        if letter.upper() in letter_to_num:
            ...
            letter_number: int = letter_to_num.get(letter.upper(), -1)

            if not to_decode:
                new_number: int = (letter_number + key_number) % len(alphabet)
            else:
                new_number = (letter_number - key_number) % len(alphabet)

            new_letter: str = num_to_letter.get(new_number, "")
            normalized_letter: str = (
                new_letter if letter == letter.upper() else new_letter.lower()
            )

            resulting_string += normalized_letter
        else:
...
def render_ui(lang: str) -> None:
    ...
    with (
        ui.input(label="⌨️ " + ("Encoded text" if is_english else "Шифротекст"))
        .classes("w-full")
        .props("filled readonly") as result_output
    ):
    ...
    def process_text(to_decode: bool = False) -> None:
