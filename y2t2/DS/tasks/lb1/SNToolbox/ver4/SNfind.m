...
if b
    figure;
    lb=uicontrol('Style','listbox','Position',[10 100 500 300],'BackgroundColor','White','SelectionHighlight','off');
else
    lb=0;
end;
flag=0;
if isnodepresent(SN.node,'?')
   my_print('Warning: The first argument contains a node [?]',lb,b);
   return;
end;
if length(SN.node)>=length(SN1.node)
   ...
   for i=1:1:length(SN1.node)
       if SN1.node{i}~='?'
          ...
          if k>0
             if SN.nodetype(k)==SN1.nodetype(i)
                ...
                my_print('Error: The first argument node [',S.node{k},'] and the second argument node [',S1.node{i},'] are nodes of different type',lb,b);
                k=0;
             end;
          end;
       else
       ...
       if k==0
             my_print('Error: The node [',SN1.node{i},'] is not exists in first argument semsntic network',lb,b);
          flag==1;
       end;
   end;
else
...
if  flag==0
    ...
    if max(h)<intmax
        ...
        if length(Rh)==0
           Res=[];
           my_print('RESULT: Goal node is not found',lb,b)
        else
           my_print(sprintf('RESULT: The number of founded solutions is %d',length(Rh)),lb,b);
           for i=1:1:length(Rh)
               Res{i}=SN.node{Rh(i)};
           end;  
        end;
    else
       my_print('Error: This function can not be be used for full connected networks',lb,b);
    end;
else
    my_print('Error: The second argument must be a subnetwork of first argument',lb,b);
end;
...
function my_print(s,lb,b)
% it is my function
if b
    set(lb,'String',[get(lb,'String');s]);
end;