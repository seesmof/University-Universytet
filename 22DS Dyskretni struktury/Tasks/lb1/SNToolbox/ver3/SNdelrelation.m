function Res=SNdelrelation(SN, node1, relation, node2)
% Function Res=SNdelrelation(SN, node1, relation, node2)
% delete a relation between the node1 and the node2 nodes
% from the semantic network SN and return updated
...
if nargin==4
   if and(and(and(isstr(node1)==1,isstr(node2)==1), isstr(relation)==1), and(strcmp(node1, node2)==0, and(strcmp(node2, relation)==0, strcmp(node1, relation)==0)))
      ...
      if n1>0
         ...
         if n2>0
             if strcmp(SN.relation{n1,n2}, relation)==1
                SN.relation{n1,n2}=[];
                Res=SN;
             else
                disp(strcat('Error: The relation [', relation, '] between the nodes [',node1,'] and [',node2,'] is not exists in the relation field'));     
             end;  
         else
