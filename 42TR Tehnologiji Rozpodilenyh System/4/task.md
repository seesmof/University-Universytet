...
Програма виконує паралельне обчислення суми рядку $\sum_{i=0}^{N-1} a_i x_i$. Для цього до кожного з $Р$ процесів за допомогою функції MPI_Scatter() передається відповідна частина масива чисел ai та xi, кожна з яких складається з M=N/P елементів. Процеси обчислюють часткові суми ряду (змінна sum), які далі за допомогою функції MPI_Reduce() приводяться до загальної суми (змінна total).

4.2.1.1 Створити програму обчислення суми ряду.

$\sum_{i} a_i \cdot \sin{x_i}$, де i=0,1,…,N; N=240 000, $x_i$ = 0.0001 \_ I, $a_i$ – випадкові дійсні числа в діапазоні (-1, 1). Функцію $\sin{x_i}$ обчислювати за допомогою розкладання її в ряд Тейлора: $\sin{x}=x-\frac{x^3}{3!}+\frac{x^5}{5!}-...+(-1)^n \frac{x^{2n+1}}{(2 \cdot n + 1)!}+...$
...
4.2.1.3 Дослідити та намалювати графіки залежності прискорення від кількості членів ряду при обчисленнях на чотирьох процесорах, змінюючи кількість членів ряду від 20000 до 240000 з дискретністю 20000. Вимірювання виконуйте 3 рази з інтервалом між вимірюваннями 15 хвилин, результати вимірювань занесіть до таблиці4.2.
