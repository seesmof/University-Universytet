What unopinionated framework to choose? For frontend React.js is the best choice because I know it already. For backend I'm choosing between Express.js, NestJS and Koa.

Gotta try Supabase for auth and database and so on.

Watched some video on youtube that says that first you should work on a frontend, then on a backend and then on a database, i think.

For comparison we can take Telegram, Discord and WhatsApp. As the example I'm taking Telegram.

To do CRUD (Create Read Update Delete) we can have such datapoints:
